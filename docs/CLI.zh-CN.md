...
V0 使用本地 Server 作为依赖根；启用 Runner 或 Tunnel 时必须同时启用 Server。Controller 不接管已经由 webcodex.service / webcodex.socket / webcodex-runner.service 管理的运行实例，检测到已有服务处于 active 状态时会拒绝启动，避免双重 ownership。
